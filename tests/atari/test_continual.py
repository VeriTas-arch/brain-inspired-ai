"""Continual training, matched task initialization, and EWC/GPM boundaries."""
...
from biai.atari.scripts import train_continual as training
...
def test_three_task_gpm_starts_fresh_accumulates_and_saves_an_evaluable_checkpoint(
    ...
    videos = sorted(tmp_path.glob("case/videos/*_training.mp4"))
    assert len(videos) == 3
    ...
    report = json.loads((tmp_path / "case/training_summary.json").read_text())
    assert report["method"] == "gpm" and report["initialization"] == "random"
    assert report["task_steps"] == dict(zip(TASKS, (4, 6, 4), strict=True))
    assert report["evaluation_steps"] == dict(zip(TASKS, ([4], [6], [4]), strict=True))
    for game, budget in zip(TASKS, (4, 6, 4), strict=True):
        evaluation = json.loads((tmp_path / "case/learning.json").read_text())[game]
        assert [row["step"] for row in evaluation["evaluations"]] == [budget]
    ...
    path = tmp_path / "case/checkpoints/final.pt"
    checkpoint = torch.load(path, weights_only=True)
    assert checkpoint["gpm"]["completed_tasks"] == list(TASKS)
    assert checkpoint["gpm"]["subspaces"]["1"]["rank"] == 3
    assert len(list((tmp_path / "case/checkpoints").glob("*.pt"))) == len(TASKS)
    for index, (spaces, actors, critics) in enumerate(boundaries, start=1):
        boundary_path = tmp_path / "case" / report["boundary_checkpoints"][index - 1]
...
def test_periodic_task_evaluation_saves_loadable_models_without_changing_budget(
    ...
    data = json.loads((tmp_path / "case/learning.json").read_text())["pong"]
    ...
    assert "checkpoint" not in data["evaluations"][0]
    assert [p.name for p in (tmp_path / "case/checkpoints").glob("*.pt")] == ["final.pt"]
    checkpoint = torch.load(tmp_path / "case/checkpoints/final.pt", weights_only=True)
    restored = TinyPPO()
...
def test_new_heads_are_matched_across_methods_and_stage_rewards_are_raw(monkeypatch, tmp_path):
    ...
    class RecordedPPO(TinyPPO):
        def register_task(self, task_id, action_dim):
            super().register_task(task_id, action_dim)
            initial_heads[method].append(
                (
                    copy.deepcopy(self.actors[task_id].state_dict()),
                    copy.deepcopy(self.critics[task_id].state_dict()),
                )
            )
    ...
    for method in ("finetune", "ewc", "gpm"):
        ...
        recorded[method] = json.loads((root / "initialization.json").read_text())
        assert not list((root / "checkpoints").glob("*initial.pt"))
    for method in ("ewc", "gpm"):
        assert recorded[method] == recorded["finetune"]
