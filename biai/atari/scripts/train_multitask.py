...
def train_multitask(
    ...
    if steps_per_game is not None:
        if steps_per_game <= 0 or steps_per_game % num_envs:
            raise ValueError("steps_per_game must be positive and divisible by num_envs")
        if total_steps != steps_per_game * len(games):
            raise ValueError("total_steps must equal steps_per_game times the number of games")
    ...
    with ExitStack() as resources:
        ...
        while step < total_steps:
            ...
            if steps_per_game is None:
                ...
                available = [game for game in games if task_steps[game] < steps_per_game]
                current_game = str(task_rng.choice(available))
                remaining = steps_per_game - task_steps[current_game]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--steps-per-game", type=int, help="Exact quota per game; --steps must equal its sum"
    )
